function [ qrs, sqi, qrs_comp, qrs_header,detect_out ] = detect_sqi_mod(data, header, fs, opt)
...
recordName = ['TMP_' datestr(now,'dd-mm-yyyy-HHMMSSFFF') num2str(randi(200))]; % add rand to avoid colision in parallel comp.
...
wrsamp((transpose(1:N)-1),data*10000,recordName,fs,10000,'32');
...
if ~isempty(idxECG)
    for m=idxECG
        ...
        gqrs(recordName,[],[],m,150,['gqrs' num2str(m)]);
        ...
        detect_out{m} = ann_jqrs{m}(:);
        detect_out{4} = ann_gqrs{m}(:);
        %=== convert to time
...
% %                     abp{m} = [];
% %                 end
%                 % ABP features compute
%                 downsampling_rate = 125;
%                 sampling_rate = fs(m);
%                 res_P1 = resample(data(:,m),downsampling_rate, sampling_rate);
%                 %col = abpfeature(res_P1,r);
%                 %[BeatQ, percent] = jSQI(col, r, res_P1);
%                 abp{m} = old_wabp(res_P1);
%                 
%         end
%         
...
% %         detect_out{m} = abp{m};
% %         abp{m} = abp{m} / fs(m);
%         [ sqi_bp{m}, header_sq, sqi_bp_values, header_sqi ] = calcABPSQI(res_P1, abp{m}, downsampling_rate);
%         detect_out{m} = abp{m};
%         abp{m} = abp{m} / downsampling_rate;
%         switch opt.DELAYALG
%             case {'crosscorr','cc'}
