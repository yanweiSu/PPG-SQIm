function [ qrs, smi, qrs_comp, qrs_header, detect_out ] = detect_regularity_mod(data, header, fs, opt)
...
%wrsamp((transpose(1:N)-1),data*10000,recordName,fs(1),10000,'16+24');

wrsamp((transpose(1:N)-1),data*10000,recordName,fs(1),10000,'32');
...
if ~isempty(idxECG)
    for m=idxECG
        ...
        gqrs(recordName,[],[],m,200,['gqrs' num2str(m)]);
        ...
        detect_out{4} = ann_gqrs{m}(:); % Suppose 1:ecg, 2:bp, 3:ppg
        %=== convert to time
