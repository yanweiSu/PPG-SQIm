...
MPA = sum(power(pks(1)-1:pks(1)+1));
for h = 2:3
    MPA = MPA + sum(power(pks(h)-1:pks(h)+1));
end

% calculate total respiratory range (TRA)
% sum of all values within respiratory range of .1Hz-.75Hz
TRA = sum(power(.1*sec:pks(1)+round(.5*sec)));
for h = 2:3
    TRA = TRA + sum(power(pks(h)-round(0.5*sec):pks(h)+round(0.5*sec)));
end
